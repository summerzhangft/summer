...
class ArticleForm(forms.ModelForm):
    ...
    class Meta:
        model = Article #根据模型创建表单
        fields = ['title','tag_list','background','description','raw_content',]#该表单包含的字段
        widgets = {
            'title': forms.TextInput(get_field_attrs('Title')),
            'background': forms.TextInput(get_field_attrs('Background Url, default {}'.format(settings.SUMMER_CONFIG.get('SITE_ARTICLE_BACKGROUND')))),
